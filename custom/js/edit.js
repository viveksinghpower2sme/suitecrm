...
var select_phonetype = '<select id="sel_'+incr_count+'" name="sel_'+incr_count+'"><option value="landline" '+landline+' >Landline</option><option value="mobile" '+mobile+'>Mobile</option><option value="fax" '+fax+'>Fax</option></select>';
checked ='';
...
if(invalid==1)
	invalid='checked';
str_append = '<tr ><td>'+select_phonetype+' <input type="text" id = "phone_'+incr_count+'" name="phone_'+incr_count+'" value="'+name+'" size="30"/>  <button onclick="javascript:remove_phoneNumber(this)" id="'+incr_count+'" class="id-ff-remove" name="'+incr_count+'" type="button" tabindex="0"><img src="index.php?entryPoint=getImage&amp;themeName=Sugar5&amp;imageName=id-ff-remove-nobg.png"></button> </td><td><input type="radio" style="vertical-align: middle" name="phoneradio" value="'+incr_count+'" '+checked+' > </td><td><input type="checkbox" name="optedout_phone_'+incr_count+'" '+optout_checked+'  value="'+incr_count+'"> </td><td><input type="checkbox" name="invalid_phone_'+incr_count+'" '+invalid+' value="'+incr_count+'"> </td></tr>';
...
function addRecords()
{
	var phone_record = JSON.parse($('#phonerecordedit').val());
