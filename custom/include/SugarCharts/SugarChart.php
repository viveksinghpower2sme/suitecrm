...
class SugarChart {
	...
	function constructURL(){
		...
		foreach ($this->url_params as $param => $value){
			...
			elseif($param == 'created_by')
			{
				$param = 'created_by_advanced[]';
			}	
			if (is_array($value)){
	...
	function xmlDataGenericChart(){
		...
		foreach ($new_data as $groupByKey => $value){
			...
			elseif($this->group_by[0] == 'rfq_state_c')
			{

				$additional_param = '&rfq_state_c_advanced=' . urlencode(array_search($groupByKey,$GLOBALS['app_list_strings']['rfq_state_c_dom']));
			}
			else{
