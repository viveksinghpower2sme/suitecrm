...
function RejQuote(dropdownKey,comm,stage,alertComment)
{
		var iReject = 0;

		var gconfrm = 	$('#'+dropdownKey+' option:selected').val();	
		var gcmt = $('#'+comm).val();
		
		if(gconfrm == stage && gcmt != ''){
				iReject = 1;
		}
		else if(gconfrm == stage && gcmt == ''){
			alert('Please enter "'+alertComment+'"');
				$('#'+comm).focus();
				iReject = 2;	
			}
...
function handleRejection(){
		...
		if($('#rfq_state_c').val() == "goods_confirmation")
		{
			return RejQuote("goods_confirmation_status_c","good_cnf_rejec_comm_c","rejected_by_scm","Good Confirmation Comments");	
		}
		if($('#rfq_state_c').val() == "finance_approval")
		{
			return RejQuote("finance_status_c","finance_rejec_comm_c","rejected_by_finance","Finance Comments");	
		}
...
function check_custom_data(action)
{
	...
	// avoid validation in case of rejection of RFQ
	var iRejectRFQ = handleRejection();
		if(iRejectRFQ == 2){
				return false;	
		}
		else if(iRejectRFQ == 1){
...
$(document).ready(function (){ 
	...
	if($('#rfq_state_c').val()=="new"){
		...
		//------------by dinesh-----------------------------------
			...
			if(c_tax_p!='CST_Only' && c_tax_p!='Excise_And_CST' && c_tax_p!='E1_Sales')
			{
...
$(function() {
	...
	if($('#rfq_state_c').val()=="sales_quote_margin" || $('#rfq_state_c').val() == "sales_quote_price")
	{
		...
		$('#payment_c').change(function(){
			...
			tmpval = Math.floor(tmpval * 1000) / 1000;//14072016
	...
	$( "#customer_taxation_preference_c" ).change(function (){
		...
		if(c_tax_p == 'E1_Sales')
		{
			$('#form_type_c').val("C_Form");
			$('#form_type_c option[value="C_Form"]').attr("selected", "selected");
			$('#form_type_c').change(function(){
				$('#form_type_c').val("C_Form");
				$('#form_type_c option[value="C_Form"]').attr("selected", "selected");
			});
...
function marginpercentchanged(k)
{
		...
		if(came_from_total_margin==0)
		{
			...
			if(lineitm1 > 0)
			{
				...
				if(!num) 
				{
					$("#total_margin_c").val("0");
				}
				else
				{
					$('#total_margin_c').val(num);
				}
				den1 = Math.floor(den1 * 1000) / 1000;
...
function calculate_margPer_totalSalesValue_totalMarginPer(k) {
		...
		liwmp2 = Math.floor(liwmp2 * 1000) / 1000;
	...
	if($('#rfq_state_c').val()!="goods_confirmation" || $('#rfq_state_c').val()=="final_scm_confirmation")
	{
			...
			for(var i=1;i<=lineitm0;i++)
		  	...
		  	if(!final_val) $('#total_sales_value_c').val('0');
		  		...
		  		final_val = Math.floor(final_val * 1000) / 1000;
...
function CalculateCustomerLoadingCharges()
{
			...
			if(lineitm1 > 0)
			{
				for(i=1;i<=lineitm1;i++)
				{
					...
					{	lc_c = Math.floor(lc_c * 1000) / 1000;
...
function CalculateCustomerFreightCharges()
{
			...
			if(lineitm1 > 0)
			{
				for(i=1;i<=lineitm1;i++)
				{
					...
					{	fc_c = Math.floor(fc_c * 1000) / 1000;
...
function formulaImplementation(k){
		...
		$('#sales_rate_per_unit_c_'+k).change(function(){
			...
			csp1 = Math.floor(csp1 * 1000) / 1000;
