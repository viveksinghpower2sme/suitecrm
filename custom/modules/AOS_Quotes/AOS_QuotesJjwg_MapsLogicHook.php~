...
class AOS_QuotesJjwg_MapsLogicHook {
    ...
    */
    ...
    private function accountContactMapping($contactId,$accountId)
    {
        global $db;
        $sql = "select id from accounts_contacts where contact_id='$contactId' and account_id='$accountId'";
        $out = $db->query($sql);  
        $result = $db->fetchRow($out);
        ...
        if(empty($result['id']))
        {
            ...
            $insSql = "insert into accounts_contacts (id,contact_id,account_id,date_modified) values ('$uniqueId','$contactId','$accountId',now())";
            $db->query($insSql); 
        }
    }
		...
		$first_key = key($phoneBean);
			...
			$scmBean= BeanFactory::getBean('Users', $bean->users_aos_quotes_2users_ida);
             ...
             $info=array("object"=>'rfq',
                ...
                "contact"=>array("mobile_no"=>$phoneBean[$first_key]->name,
		...
		$first_key = key($phoneBean);
            ...
            foreach($address as $key=>$value){
             ...
             $addInfo = array("CustomerContact"=>
                                array("cNo"=>$contactBean->contactnavid_c,
                                    "cName"=>$contactBean->first_name." ".$contactBean->last_name,
                                    "cPhone"=>$phoneBean[$first_key]->name,
                                    "cType"=> '1',
