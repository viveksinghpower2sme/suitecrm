...
class ViewConvertLead extends SugarView
{
	...
	public function display()
    {
        ...
        $filter .= '&leadid_c_advanced=' . urlencode($this->focus->leadid_c);
...
//        echo "<pre>";
//        print_r($this->contact->field_defs);die;
        ...
        )
    {
    	if ($lead->status == "Converted")
    	{
    		...
    		$dupes = array();
    		 $q = "SELECT id, first_name, last_name FROM contacts inner join contacts_cstm on (contacts.id=contacts_cstm.id_c) where contacts_cstm.leadid_c = '{$lead->id}' AND deleted = 0";
