...
class PendingLeadDashlet extends DashletGenericChart
{
    ...
    public function display()
    {
        ...
        $chartDef = $chartDefs['pending_lead'];
        ...
        $sugarChart->data_set = $sugarChart->sortData($sugarChart->data_set, 'm', false, 'lead_source', true, true);
	...
	//$qry = "select "
      ...
      //  $query = "select lead_source from leads group by lead_source";      
         $query = "SELECT lead_source,2 as total,count(*) as opp_count,".
            db_convert('leads.date_modified','date_format',array("'%Y-%m'"),array("'YYYY-MM'"))." as m ".
           " FROM leads";
       
        if (count($this->obm_ids) > 0)
            $query .= " AND opportunities.assigned_user_id IN ('" . implode("','",$this->obm_ids) . "')";
        $query .= " GROUP BY lead_source,".
                        db_convert('leads.date_modified','date_format',array("'%Y-%m'"),array("'YYYY-MM'")) .
                    " ORDER BY m";



	//------------------------------------------------------------
	//$query = "select assigned_user_id , 1 as total , count(*) as opp_count,DATE_FORMAT(calls.date_modified,'%Y-%m') as m frm calls where assigned_user_id='".$_SESSION["authenticated_user_id"]."' and status = 'Planned' group by assigned_user_id , DATE_FORMAT(calls.date_modified,'%Y-%m') ORDER BY m" ;
	//-----------------------------------------------------------
	//echo "<pre>";echo $_SESSION["authenticated_user_id"];die;
        //echo $query;die;         
