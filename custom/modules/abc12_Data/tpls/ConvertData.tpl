...
{{/if}}
 {if !empty($def.default_action) && $def.default_action == "create"}
     {if $data_conv_activity_opt == 'move' || $data_conv_activity_opt == 'copy' || $data_conv_activity_opt == ''}
        YAHOO.util.Event.onContentReady('data_conv_ac_op_sel', function(){ldelim}
     {else}
...
{*{sugar_translate label='{{$label}}' module='abc12_Data'}*}
</td><td>
   
{{if !empty($selectRelation)}}
{if !empty($def.select)}
    {*{sugar_translate label='LNK_SELECT_{{$module|strtoupper}}' module='abc12_Data'}
    {if $def.required }
        <span class="required">{{$APP.LBL_REQUIRED_SYMBOL}}</span>
    {/if}*}
</td><td id ="select{{$module}}">
   
{*{sugar_field parentFieldArray='lead_def' vardef=$lead_def[$selectRelation] displayType='EditView' displayParams=$displayParams formName=$form_name call_back_function='set_return_lead_conv'}*}
<script>
...
sqs_objects['{{$form_name}}_{$selectFields.{{$module}}}'] = {ldelim}
    ...
    group         : 'or',
    field_list    : ['name', 'id'],
    populate_list : ['{$selectFields.{{$module}}}', '{$lead_def[$selectFields.{{$module}}].id_name}'],
    conditions    : [{ldelim}'name':'name','op':'like','end':'%','value':''{rdelim}],
    required_list : ['{$lead_def[$selectFields.{{$module}}].id_name}'],
    order         : 'name',
    limit         : '10'
{rdelim}
</script>
