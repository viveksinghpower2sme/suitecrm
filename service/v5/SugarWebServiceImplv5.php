...
require_once('service/getRegionInfo.php');

class SugarWebServiceImplv5 extends SugarWebServiceImplv4_1 {

    public function __construct()
    {
        self::$helperObject = new SugarWebServiceUtilv5();
    }
	function createRfq($session,$objDetails,$addDetails,$rfqDetails){
		
		
	}
	
	function create_address($session,$objDetails,$addDetails){
		if(empty($addDetails['type'])){
			
			$error=$this->errorMessage("Please mention the address type");
						return $error;
		}
		$regionObj = new getRegionInfo($GLOBALS['db']->connectOptions);
		$regionJson=$regionObj->regionDetails($addDetails['postalcode']);
		$regioninfo = json_decode($regionJson,true);
		
	if($objDetails['module']=='Contacts'){
			...
			$name_value_list=array(
					'0' => array(
			         ...
			         '1' => array(
			            'name' => "postcode_c",
			            'value' => $addDetails['postalcode']
			        ),
					 '2' => array(
			            'name' => "address_city",
			            'value' => $regioninfo['city']
			        ),
					 '3' => array(
			            'name' => "address_state",
			            'value' =>  $regioninfo['state']
			        ), 
			        '4' => array(
			            'name' => "address1_c",
					...
					'5' => array(
			            'name' => "address_type_c",
			            'value' =>  $addDetails['type']
			        )
				);
		$addId= $this->set_entry($session, $module_name, $name_value_list);
		 if(!empty($addId['id'])){
		 	 $modDetials= $this->set_relationship($session, 'Contacts', $objDetails['id'],'contacts_fp_event_locations_1',$addId['id'],$list);
			$addDetails['id']=$addId['id'];
			return $addDetails; 
	...
	elseif($objDetails['module']=='Leads') {
		
		
		if($addDetails['type']=='billing_address')
		{
			$name_value_list=array('0' => array(
				            'name' => "id",
				            'value' => $objDetails['id']
				        ),
				         '1' => array(
		...
		elseif($addDetails['type']=='shiping_address'){
		...
		$leadDetails =$this->set_entry($session, 'Leads', $name_value_list);
		
		if(!empty($leadDetails['id'])){
				return $addDetails;
		}
	}
	...
	function delete_address($session,$objDetails,$addDetails){
		
		if($objDetails['module']=='Contacts'){
			$module_name ='FP_Event_Locations';
			$deleted ='1';
			$name_value_list=array(
					'0' => array('name' => "id",'value' => $addDetails['id'] ),
					'1' => array('name' => "deleted",'value' =>	$deleted )
				
);
			$addId= $this->set_entry($session, $module_name, $name_value_list);
	/*	print_r($addId); die;
			$link_field_name='contacts_fp_event_locations_1';
		$addId= $this->set_relationship($session, $objDetails['module'], $objDetails['id'], $link_field_name, $addDetails['id'], array(), $deleted);*/
		print_r($addId); die;
		}
		else{
			$error=$this->errorMessage("addresses are not deleted for ".$objDetails['module']);
						return $error;
	...
	function get_phone_detail($session,$phoneNo,$select_fields=array())
	{
		$module_name = '';
		$id ='';
		$track_view = FALSE;	
		$details =array();
		$a = checkPhone($phoneNo,1);
		$Data = json_decode($a);
		print_r($Data);die;
		if(!isset($Data->id) || (isset($Data->id) && empty($Data->id)) || (isset($Data->id) && !empty($Data->id) && $Data->module =='abc12_Data' )){
			$error=$this->errorMessage("This No. Doesnot exist in contact or lead module in CRM");
			return $error;
		...
		if($module_name=='Contacts'){
			$select_fields=array("id", "first_name","last_name","email1","company_name_c");
			
			$modDetials= $this->get_entry($session, $module_name, $id,$select_fields);
			
			$details['contactDetails']['id'] = $modDetials['entry_list'][0]['id'];
			$details['contactDetails']['module'] = $modDetials['entry_list'][0]['module_name'];
			$details['contactDetails']['first_name'] = $modDetials['entry_list'][0]['name_value_list']['first_name']['value'];
			$details['contactDetails']['last_name']=$modDetials['entry_list'][0]['name_value_list']['last_name']['value'];
			$details['contactDetails']['email_id']=$modDetials['entry_list'][0]['name_value_list']['email1']['value'];
			$details['contactDetails']['mobile_number']=$phoneNo;
			$details['contactDetails']['company_name']=$modDetials['entry_list'][0]['name_value_list']['company_name_c']['value'];
			
			// get user info	
			$userInfo = $this->custom_get_user_info($session, $module_name, $id);
			$details['Kam']['id'] = $userInfo['entry_list'][0]['id'];
			$details['Kam']['first_name'] = $userInfo['entry_list'][0]['name_value_list']['first_name']['value'];
			$details['Kam']['last_name']=$userInfo['entry_list'][0]['name_value_list']['last_name']['value'];
			$details['Kam']['email_id']=$userInfo['entry_list'][0]['name_value_list']['email1']['value'];
			$details['Kam']['mobile_number']=$userInfo['entry_list'][0]['name_value_list']['phone_mobile']['value'];
			
			// get account info
			$relatedAccountInfo = $this->custom_get_account_info($session, $module_name, $id);
			for($i=0; $i<sizeof($relatedAccountInfo['entry_list']); $i++){
				
				$details['associated_organisation'][$i]['sme_id'] = $relatedAccountInfo['entry_list'][$i]['id'];
				$details['associated_organisation'][$i]['company_name'] = $relatedAccountInfo['entry_list'][$i]['name_value_list']['name']['value'];
			}	
			
			// get address info
			$relatedAddressInfo = $this->custom_get_address_info($session, $module_name, $id);
			for($i=0; $i<sizeof($relatedAddressInfo['entry_list']); $i++){
				$details['address'][$i]['id'] = $relatedAddressInfo['entry_list'][$i]['id'];
				$details['address'][$i]['type'] = $relatedAddressInfo['entry_list'][$i]['name_value_list']['address_type_c']['value'];
	
				$details['address'][$i]['address'] = $relatedAddressInfo['entry_list'][$i]['name_value_list']['address']['value'];
				$details['address'][$i]['postalcode'] = $relatedAddressInfo['entry_list'][$i]['name_value_list']['address_postalcode']['value'];
		
			}				
		}
		else if($module_name=='Leads')
		{
			$select_fields=array("id", "first_name","last_name","email1","primary_address_street",
			"primary_address_postalcode","alt_address_street","alt_address_postalcode",'account_name');
			$modDetials= $this->get_entry($session, $module_name, $id,$select_fields);
			...
			$details['contactDetails']['email_id']=$modDetials['entry_list'][0]['name_value_list']['email1']['value'];
			$details['contactDetails']['mobile_number']=$phoneNo;
			$details['contactDetails']['company_name']=$modDetials['entry_list'][0]['name_value_list']['account_name']['value'];
			...
			//print_r($modDetials); die;
			$details['address'][0]['id'] = null;
			$details['address'][0]['type'] = 'billing_address';
			$details['address'][0]['address'] = $modDetials['entry_list'][0]['name_value_list']['primary_address_street']['value'];
			$details['address'][0]['postalcode'] = $modDetials['entry_list'][0]['name_value_list']['primary_address_postalcode']['value'];
			$details['address'][1]['id'] = null;
			$details['address'][1]['type'] = 'shiping_address';
			$details['address'][1]['address'] = $modDetials['entry_list'][0]['name_value_list']['alt_address_street']['value'];
			$details['address'][1]['postalcode'] = $modDetials['entry_list'][0]['name_value_list']['alt_address_postalcode']['value'];
    ...
    */
	...
	private function custom_get_user_info($session, $module_name, $id){
		$link_field_name = 'assigned_user_link';
			...
			$val=$this->get_relationships($session,$module_name,$id,$link_field_name,$related_module_query,$related_fields,$related_module_link_name_to_fields_array, $deleted);
			return $val;
	}
	private function custom_get_account_info($session, $module_name, $id){
		...
		private function custom_get_address_info($session, $module_name, $id){
			$link_field_name = 'contacts_fp_event_locations_1';
